...
resource "aws_customer_gateway" "ToAzureInstance1" {
  ...
  # Azure VPN 게이트웨이의 ASN
  ip_address = azurerm_public_ip.gwpip1.ip_address
  # Azure VPN의 첫번째 공용 IP
...
resource "aws_customer_gateway" "ToAzureInstance2" {
  bgp_asn    = var.azure_asn
  # Azure VPN 게이트웨이의 ASN
  ip_address = azurerm_public_ip.gwpip2.ip_address
  # Azure VPN의 두번째 공용 IP
  type       = var.aws_customer_gw.type
